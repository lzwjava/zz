...
Tokenizes 'content' column with GPT-2 BPE (tiktoken), produces:
...
into numpy uint16 array (no Python list phase). Single-process to avoid
multiprocessing memory duplication (each parquet file produces ~370M tokens
= ~10 GB in Python list form, instant OOM with 16 workers).
...
def main():
    ...
    for fi, pq_path in enumerate(parquet_files):
        ...
        for batch in pf.iter_batches(columns=["content"], batch_size=8192):
            for val in batch.column("content"):
                ...
                # EOT separator + code tokens — write directly to numpy buffer
