...
Resumable: state is tracked in <output-dir>/progress.json. Cancel any time
...
def build_or_load_state(output_dir: str, refresh: bool) -> dict:
    ...
    if not all_shards:
        ...
        sys.exit(1)
    selected = select_shards(all_shards, int(TARGET_TOKENS * BYTES_PER_TOKEN))
    state = {
        "repo_id": REPO_ID,
