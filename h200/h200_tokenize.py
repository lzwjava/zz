#!/usr/bin/env python3
"""Tokenize FineWeb-Edu parquet shards into uint16 token shards for GPT-3 ablation.

Reads:  /workspace/zz/datasets/fineweb-edu/data__*.parquet
Writes: /workspace/zz/datasets/tokenized/edufineweb_{split}_{idx:06d}.npy

Output format matches nanoGPT/data/fineweb/prepare_fineweb.py:
  - Shard 0 -> val, rest -> train
  - 100M uint16 tokens per shard
  - GPT-2 BPE encoding
  - Each doc prefixed with <|endoftext|>
"""
...
INPUT_DIR = "/workspace/zz/datasets/fineweb-edu"
OUTPUT_DIR = "/workspace/zz/datasets/tokenized"
...
def main():
    ...
    nprocs_default = max(1, (os.cpu_count() or 2) // 2)
    ap.add_argument("--nprocs", type=int, default=nprocs_default)
    args = ap.parse_args()
    ...
    print(f"  ~{shard_index * args.shard_size:,} total tokens tokenized")
    return 0
